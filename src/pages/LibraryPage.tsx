...
import { deleteRating } from '../db/queries'
...
export function LibraryPage() {
  ...
  const ratings = useLiveQuery(
    () => db.ratings.orderBy('ratedAt').reverse().toArray(),
    []
  ) ?? []
          ...
          {(['all', 'movie', 'tv'] as MediaFilter[]).map((f) => (
            <button
              ...
              className={`flex-1 py-2 rounded-xl text-sm font-semibold transition-all capitalize ${
                mediaFilter === f ? 'bg-white text-black shadow' : 'text-gray-400'
              }`}
            >
            ...
            {([0, 1, 2, 3, 4, 5] as ScoreFilter[]).map((s) => (
              <button
                ...
                className={`w-8 h-8 rounded-lg text-xs font-semibold transition-all ${
                  minScore === s ? 'bg-amber-400 text-black' : 'bg-white/8 text-gray-400'
                }`}
              >
                {s === 0 ? 'All' : `${s}★`}
              </button>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-10 flex flex-col gap-2.5">
        {filtered.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 text-sm mt-1">
              {ratings.length === 0 ? 'Go rate some titles to build your library' : 'Try a different filter'}
            </p>
            {ratings.length === 0 && (
              <button
                type="button"
        ...
        {filtered.map((r) => (
          <button
            key={r.id}
            type="button"
            onClick={() => { setEditing(r); setPendingScore(r.score) }}
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-5">
              {editing.posterPath && (
                <img
                  src={`${POSTER_BASE}${editing.posterPath}`}
                  alt={editing.title}
            ...
            <button
              type="button"
              onClick={() => navigate(`/detail/${editing.mediaType}/${editing.tmdbId}`)}
            ...
            <button
              ...
              onClick={() => void handleUpdateRating()}

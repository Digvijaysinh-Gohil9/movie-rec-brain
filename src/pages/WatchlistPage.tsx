...
import { removeFromWatchlist, rateFromWatchlist, addToWatchlist } from '../db/queries'
import { useWatchlist } from '../hooks/useDb'
    ...
    <div className="fixed inset-0 bg-black/90 z-50 flex flex-col">
      <div className="px-5 pt-14 pb-4 flex items-center gap-3">
        <input type="search" value={query} onChange={(e) => setQuery(e.target.value)}
          ...
          autoFocus />
        <button type="button" onClick={onClose} className="text-gray-400 text-sm px-3 py-2 shrink-0">Cancel</button>
      </div>
      <div className="flex-1 overflow-y-auto px-5 flex flex-col gap-2.5 pb-10">
        {loading && [1,2,3].map((i) => <div key={i} className="h-20 rounded-2xl bg-white/5 animate-pulse" />)}
        {!loading && query && results.length === 0 && <p className="text-gray-500 text-sm text-center py-10">No results found</p>}
        {!loading && results.map((r) => {
          ...
          return (
            <button key={`${r.media_type}:${r.id}`} type="button"
              onClick={() => void handleAdd(r)} disabled={adding === r.id}
              ...
              <div className="flex-1 flex flex-col justify-center min-w-0">
                ...
                <p className="text-xs text-gray-400 mt-0.5">{year && <span>{year} · </span>}<span>{r.media_type === 'tv' ? 'TV Show' : 'Movie'}</span></p>
              </div>
              <div className="flex items-center shrink-0"><span className="text-2xl">{adding === r.id ? '…' : '＋'}</span></div>
            </button>
          )
        })}
      </div>
    ...
    <div className="min-h-screen bg-[#0f0f0f] flex flex-col">
      ...
      {watchlist.length === 0 && (
        <div className="flex-1 flex flex-col items-center justify-center px-10 text-center pb-20">
          ...
          <p className="text-gray-500 text-sm mt-2">Tap <strong className="text-gray-400">+ Add</strong> to save movies or shows someone recommended</p>
          <button type="button" onClick={() => setShowAdd(true)}
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-28 flex flex-col gap-3">
        {watchlist.map((entry) => (
          <div key={entry.id} className="flex gap-3 p-3 rounded-2xl bg-white/5">
            <button type="button" onClick={() => navigate(`/detail/${entry.mediaType}/${entry.tmdbId}`)}
            ...
            <div className="flex flex-col items-center justify-center gap-2 shrink-0">
              <button type="button" onClick={() => { setRating(entry); setPendingScore(null) }}
                className="w-9 h-9 rounded-xl bg-amber-400/15 flex items-center justify-center text-amber-400 text-lg active:bg-amber-400/30"
                title="Mark as watched">✓</button>
              <button type="button" onClick={() => void handleRemove(entry)}
                className="w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center text-gray-600 text-base active:bg-white/10"
                title="Remove">✕</button>
            </div>
          </div>
        ))}
      ...
      {rating && (
        <div className="fixed inset-0 bg-black/80 flex items-end z-50" onClick={() => setRating(null)}>
          <div className="w-full bg-[#1a1a1a] rounded-t-3xl p-6 pb-10" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-5">
              {rating.posterPath && <img src={`${POSTER_BASE}${rating.posterPath}`} alt={rating.title} className="w-12 h-16 rounded-xl object-cover" />}
              <div>
            ...
            <button type="button" disabled={!pendingScore || saving} onClick={() => void handleRate()}
